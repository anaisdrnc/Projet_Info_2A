...
class PlaceOrderView(VueAbstraite):
    def choisir_menu(self):
        ...
        while choice == "add a product to the order" and nb_iterations < 50:
            list_products_proposed = {product for product in list_products if product not in list_choosen_products_names}
            product = inquirer.select(message="Choose a product : ", choices=list_products_proposed,).execute()
            quantity = inquirer.number(message= "Quantity :").execute()
            list_choosen_products_names.append(product)
            quantities.append(quantity)
            total_amount += prices[product]*quantity
        ...
        address_order = AddressService.add_address(address, city, postal_code)
